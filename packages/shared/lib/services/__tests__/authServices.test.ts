...
describe('validateToken', () => {
  ...
  it('returns "invalid" when the response is 401 Unauthorized', async () => {
    vi.mocked(fetch).mockResolvedValue(fakeResponse(false, null, 401, 'Unauthorized'));

    expect(await validateToken(TOKEN, VALIDATE_URL)).toBe('invalid');
  });

  it('returns "invalid" when response is ok but user_id is missing from the body', async () => {
  ...
  it('returns "unreachable" when fetch throws a network error', async () => {
    ...
    expect(await validateToken(TOKEN, VALIDATE_URL)).toBe('unreachable');
  });
