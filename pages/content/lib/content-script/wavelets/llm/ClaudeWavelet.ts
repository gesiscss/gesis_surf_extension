  ...
  extractMessage(element: HTMLElement): LLMData | null {
    try {
      ...
      const messageContent = isUser
        ? element.querySelector(this.sel(element, 'user_content', '.whitespace-pre-wrap'))?.textContent?.trim() ||
      ...
      const allMessages = Array.from(document.querySelectorAll<HTMLElement>(`${userContainerSel}, [${streamingAttr}]`));
