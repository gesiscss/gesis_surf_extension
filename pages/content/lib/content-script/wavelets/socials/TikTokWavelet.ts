...
export class TikTokWavelet extends BaseSocialWavelet {
    ...
    extractPost(container: HTMLElement): (TikTokPostData & SocialPostData) | null {
        try {
            ...
            const playerWrapper = container.querySelector<HTMLElement>('[id^="xgwrapper-"]');
            ...
            const displayName = container.querySelector<HTMLElement>('[data-e2e="video-author-avatar"] ~ * p')
                ?.textContent?.trim() ?? authorHandle;

            // Verified = teal checkmark svg present near creator info
            const isVerified = !!container.querySelector('[data-e2e="video-author-avatar"] ~ * path[fill="#20D5EC"], [data-e2e="video-author-avatar"] ~ * path[fill="#20d5ec"]');

            // Caption: join all desc-span elements
            const caption = Array.from(container.querySelectorAll('[data-e2e^="desc-span"]'))
            ...
            const likesText    = container.querySelector('[data-e2e="like-count"]')?.textContent?.trim() ?? '0';
            const commentsText = container.querySelector('[data-e2e="comment-count"]')?.textContent?.trim() ?? '0';
            const sharesText   = container.querySelector('[data-e2e="share-count"]')?.textContent?.trim() ?? '0';

            // Favorites: data-e2e is "undefined-count" on TikTok's side, parse from aria-label instead
            const favLabel = container.querySelector('button[aria-label*="Favorites"]')?.getAttribute('aria-label') ?? '';

            // Music
            const musicLink = container.querySelector<HTMLAnchorElement>('[data-e2e="video-music"]');
    ...
    protected processAddedNode(el: HTMLElement): void {
        ...
        const feedSections: HTMLElement[] = el.getAttribute('data-e2e') === 'feed-video'
            ? [el]
            : Array.from(el.querySelectorAll<HTMLElement>('[data-e2e="feed-video"]'));

        for (const section of feedSections) {
            // Walk up to the outer flex container that holds both media and action bar
            const container = section.closest<HTMLElement>('div[class*="DivContentFlexLayout"]')
