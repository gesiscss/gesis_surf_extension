...
export class TikTokWavelet extends BaseSocialWavelet {
    ...
    extractPost(container: HTMLElement): (TikTokPostData & SocialPostData) | null {
        try {
            ...
            const playerWrapper = container.querySelector<HTMLElement>(this.sel(container, 'player_wrapper', '[id^="xgwrapper-"]'));
            ...
            const avatarLink = container.querySelector<HTMLAnchorElement>(this.sel(container, 'author_avatar', '[data-e2e="video-author-avatar"]'));
            ...
            const displayName = container.querySelector<HTMLElement>(this.sel(container, 'display_name', '[data-e2e="video-author-avatar"] ~ * p'))
                ?.textContent?.trim() ?? authorHandle;

            // Verified = teal checkmark svg present near creator info
            const isVerified = !!container.querySelector(this.sel(container, 'verified_check', '[data-e2e="video-author-avatar"] ~ * path[fill="#20D5EC"]'));

            // Caption: join all desc-span elements
            const caption = Array.from(container.querySelectorAll(this.sel(container, 'caption_span', '[data-e2e^="desc-span"]')))
            ...
            const likesText    = container.querySelector(this.sel(container, 'like_count',    '[data-e2e="like-count"]'))?.textContent?.trim() ?? '0';
            const commentsText = container.querySelector(this.sel(container, 'comment_count', '[data-e2e="comment-count"]'))?.textContent?.trim() ?? '0';
            const sharesText   = container.querySelector(this.sel(container, 'share_count',   '[data-e2e="share-count"]'))?.textContent?.trim() ?? '0';

            // Favorites: data-e2e is "undefined-count" on TikTok's side, parse from aria-label instead
            const favLabel = container.querySelector(this.sel(container, 'favorites_button', 'button[aria-label*="Favorites"]'))?.getAttribute('aria-label') ?? '';

            // Music
            const musicLink = container.querySelector<HTMLAnchorElement>(this.sel(container, 'music_link', '[data-e2e="video-music"]'));
            const musicHref = musicLink?.getAttribute('href') ?? '';
            const musicIdMatch = musicHref.match(/-(\d+)$/);
            const musicId = musicIdMatch ? musicIdMatch[1] : '';
            const musicName = musicLink?.getAttribute('aria-label')
    ...
    protected processAddedNode(el: HTMLElement): void {
        ...
        for (const section of feedSections) {
            ...
            const container = section.closest<HTMLElement>(contentFlexSel)
