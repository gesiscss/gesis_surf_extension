...
function calculateScrollMetrics() {
  ...
  return {
    ...
    reading_zone:
      scrollDepth < 25 ? 'header' : scrollDepth < 50 ? 'upper_content' : scrollDepth < 75 ? 'lower_content' : 'footer',
  };
}
...
function sendScrollData(): void {
  ...
  console.log(
    `📜[Scroll] SCROLL Depth: ${metrics.scroll_depth_percentage.toFixed(2)}% | Zone: ${metrics.reading_zone} | Events: ${metrics.scroll_events}`,
  );
      ...
      .sendMessage({
        ...
        data: {
          ...
          scroll_metrics: {
            ...
            scroll_events: scrollSession.scrollEvents,
            engagement:
              scrollSession.maxDepth > 75
                ? 'high'
                : scrollSession.maxDepth > 50
                  ? 'medium'
                  : scrollSession.maxDepth > 25
                    ? 'low'
                    : 'minimal',
          },
        },
