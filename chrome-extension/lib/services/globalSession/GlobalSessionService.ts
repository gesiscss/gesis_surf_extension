...
class GlobalSessionService {
   ...
   */
  public async closeGlobalSession(global_session: GlobalSessionTypes): Promise<GlobalSessionTypes> {
    try {
   ...
   */
  public async createGlobalSession(): Promise<GlobalSessionTypes> {
    try {
  ...
  public async getLatestActiveSession(): Promise<GlobalSessionTypes | null> {
    ...
    const isSessionBeingClosed =
      session && this.sessionBeingClosed && session.global_session_id === this.sessionBeingClosed;

    // Check if the session is active and not being closed
    if (
      session &&
      !session.closing_time &&
      !isSessionBeingClosed &&
